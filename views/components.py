...
class AppHeader(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        try:
            logo_img_path = Path(__file__).parent.parent / "images" / "logo.png"
            logo_img = ctk.CTkImage(light_image=Image.open(logo_img_path), size=(160, 80))
        ...
        ctk.CTkButton(right_frame, text="Mis Chats", command=controller.show_all_chats_page, **btn_kwargs).pack(side="left", padx=5) # NUEVO BOTÓN
    ...
    def search_product(self, event=None): 
        self.controller.show_main_shop_page(search_term=self.search_entry.get())

# ... (El resto del archivo, como ProductCard, no necesita cambios)
    ...
    def __init__(self, parent, product, controller, show_cart_remove=False):
        ...
        if not is_owner:
            ...
            if hasattr(controller, 'get_cart_status'):
                try: in_cart = controller.get_cart_status(product['id'])
                except Exception: in_cart = False
            
            if show_cart_remove:
                cart_btn = ctk.CTkButton(btns_frame, text="Quitar del carrito", width=120, height=36, font=theme.FUENTE_NORMAL, fg_color="#D32F2F", hover_color="#B71C1C", command=lambda: controller.handle_remove_from_cart(product['id']))
                cart_btn.pack(side="left", padx=(0, 8))
            else:
                cart_btn = ctk.CTkButton(btns_frame, text=("En carrito" if in_cart else "Agregar al carrito"), width=120, height=36, font=theme.FUENTE_NORMAL, fg_color=(theme.COLOR_PRIMARIO if not in_cart else "#AAAAAA"), hover_color=(theme.COLOR_PRIMARIO_HOVER if not in_cart else "#888888"), state=("disabled" if in_cart or product.get('vendido', 0) == 1 or product.get('vendido', False) else "normal"), command=lambda: controller.handle_add_to_cart(product['id']))
                cart_btn.pack(side="left", padx=(0, 8))
        
        show_buy_btn = not is_owner
        if show_buy_btn:
            buy_btn = ctk.CTkButton(btns_frame, text="Comprar", font=theme.FUENTE_BOLD, fg_color=theme.COLOR_PRIMARIO, hover_color=theme.COLOR_PRIMARIO_HOVER, state=("disabled" if product.get('vendido', 0) == 1 or product.get('vendido', False) else "normal"), command=lambda: controller.handle_buy_product(product['id']))
